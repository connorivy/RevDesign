...
export default class Viewer extends EventEmitter {
  ...
  initGui() {
    ...
    this.gui.add( param, 'wireframe' ).onChange( function ( val ) {
      v.applyFilter({filterBy: {'speckle_type': 'SpeckMesh'}, wireframe: val })
      // v.sceneManager.sceneObjects.filteringManager.filterAndColorMeshObject({filterBy: {'speckle_type': 'SpeckMesh'}, wireframe: val })
      v.applyFilter(null)
    } );

    this.gui.add( param, 'color by', { 'none': null, 'applied loads': 'applied_loads', 'displacement': 'displacements', 'reactions': 'reactions' } ).onChange( function ( val ) {
      console.log(this)
      v.applyFilter({filterBy: {'speckle_type': 'SpeckMesh'}, colorBy: {property: val, direction: this.object['direction'], type: 'FEM'} })
      v.applyFilter(null)
    } );

    this.gui.add( param, 'direction', { 'Magnitude': 2, 'X': 0, 'Y': 1 } ).onChange( function ( val ) {
      v.applyFilter({filterBy: {'speckle_type': 'SpeckMesh'}, colorBy: {property: this.object['color by'], direction: val, type: 'FEM'} })
      v.applyFilter(null)
    } );

    this.gui.add( param, 'deform', 0, 5000 ).onChange( function ( val ) {
